  ...
  return (
    <div className="min-h-screen bg-surface-950 text-surface-100">
      <header className="border-b border-surface-800 px-6 py-3 flex items-center justify-between">
        ...
        <button onClick={() => navigate("/dashboard")} className="text-sm text-surface-400 hover:text-surface-200 transition-colors">Back to Dashboard</button>
      </header>
      ...
      <main className="max-w-2xl mx-auto p-6 space-y-10">
        ...
        <section>
          ...
          {createdAt && <p className="text-xs text-surface-500 mb-4">Member since {createdAt}</p>}
          <form onSubmit={handleProfile} className="space-y-4">
            <div>
              <label className="block text-sm text-surface-300 mb-1">Email</label>
              <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                className="w-full px-3 py-2 bg-surface-800 border border-surface-700 rounded-lg text-surface-100 placeholder-surface-500 focus:outline-none focus:border-blue-500 text-sm" />
            </div>
            <div>
            ...
            <button type="submit" disabled={profileLoading}
              className="px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:bg-surface-700 disabled:text-surface-500 text-white rounded-lg text-sm transition-colors">
        ...
        <section>
          ...
          <form onSubmit={handlePassword} className="space-y-4 max-w-sm">
            <div>
              <label className="block text-sm text-surface-300 mb-1">Current password</label>
        ...
        <section className="border border-red-900/50 rounded-lg p-5">
          ...
          <p className="text-sm text-surface-400 mb-4">Permanently delete your account and all associated data.</p>
          <div className="space-y-3">
            <input type="text" value={deleteConfirm} onChange={e => setDeleteConfirm(e.target.value)}
              placeholder='Type "DELETE" to confirm'
              className="w-full max-w-xs px-3 py-2 bg-surface-800 border border-surface-700 rounded-lg text-surface-100 placeholder-surface-500 focus:outline-none focus:border-red-500 text-sm" />
            {deleteErr && <p className="text-red-400 text-sm">{deleteErr}</p>}
            <button onClick={handleDelete} disabled={deleteConfirm !== "DELETE"}
              className="block px-4 py-2 bg-red-700 hover:bg-red-600 disabled:bg-surface-700 disabled:text-surface-500 text-white rounded-lg text-sm transition-colors">
              Delete my account
            </button>
          </div>
        </section>

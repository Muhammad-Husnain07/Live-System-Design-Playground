  ...
  return (
    <div className="bg-surface-900 border border-surface-800 rounded-lg p-4 flex flex-col gap-3 hover:border-surface-600 transition-colors">
      <div className="flex items-start justify-between gap-2">
        <h3 className="text-sm font-semibold text-surface-100 leading-snug">{challenge.title}</h3>
        <span className={`text-[10px] font-medium px-2 py-0.5 rounded shrink-0 ${DIFFICULTY_COLORS[challenge.difficulty] || "text-surface-400 bg-surface-800"}`}>
      ...
      <div className="flex items-center justify-between mt-auto">
        ...
        <button
          onClick={handleStart}
          disabled={starting}
          className="px-3 py-1.5 text-[11px] font-medium bg-blue-500/20 text-blue-400 hover:bg-blue-500/30 rounded transition-colors disabled:opacity-50"
        >
      ...
      <main className="flex-1 p-6 max-w-4xl mx-auto w-full">
        ...
        {challengesLoading ? (
          ...
          <div className="text-center py-20">
            <p className="text-sm text-red-400 mb-2">{challengesError}</p>
            <button onClick={fetchChallenges} className="text-xs text-blue-400 hover:text-blue-300 transition-colors">
              Retry
            </button>
          </div>
        ) : challenges.length === 0 ? (
          <div className="text-center py-20">
            <p className="text-xs text-surface-500">No challenges available yet.</p>
          </div>
        ) : (
